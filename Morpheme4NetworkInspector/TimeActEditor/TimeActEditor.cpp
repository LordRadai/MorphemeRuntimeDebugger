...
TimeActEditor::TimeActTrack::TimeActTrack(sEventGroup* src)
{
	...
	{
		this->m_event.push_back(Event{ Math::timeToFrame(src->tae_data[i]->start_time, 60), Math::timeToFrame(src->tae_data[i]->end_time - src->tae_data[i]->start_time, 60), (int)src->tae_data[i]->event_data->value,  new TimeActEventData });
...
void TimeActEditor::TimeActTrack::SaveTimeActTrack()
{
	...
	*this->m_source->group_id = this->m_eventGroup;
	...
	{
		this->m_source->tae_data[i]->start_time = Math::frameToTime(this->m_event[i].m_frameStart, 60);
		this->m_source->tae_data[i]->end_time = Math::frameToTime(this->m_event[i].m_frameStart + this->m_event[i].m_duration, 60);
...
TimeActEditor::TimeActEditor() 
{
	...
	INIReader reader(".//MorphemeNetworkInspector//res//color//timeact.ini");
