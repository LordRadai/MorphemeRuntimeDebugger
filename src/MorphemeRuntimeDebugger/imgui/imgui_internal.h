...
enum ImGuiScrollFlags_
{
    ...
    ImGuiScrollFlags_KeepVisibleEdgeX = 1 << 0,       // If item is not visible: scroll as little as possible on X axis to bring item back into m_view [default for X axis]
    ImGuiScrollFlags_KeepVisibleEdgeY = 1 << 1,       // If item is not visible: scroll as little as possible on Y axis to bring item back into m_view [default for Y axis for windows that are already visible]
...
struct ImGuiTableColumn
{
    ...
    bool                    IsUserEnabled;                  // Is the column not marked Hidden by the user? (unrelated to being off m_view, e.g. clipped by scrolling).
    bool                    IsUserEnabledNextFrame;
    bool                    IsVisibleX;                     // Is actually in m_view (e.g. overlapping the host window clipping rectangle, not scrolled).
    bool                    IsVisibleY;
