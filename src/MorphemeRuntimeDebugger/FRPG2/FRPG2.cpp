...
uint64_t FRPG2::getPlayerCtrl()
{
	...
	if (*(uint64_t*)(g_gameManagerImp + 0xD0))
		return *(uint64_t*)(g_gameManagerImp + 0xD0);
...
uint64_t FRPG2::getLockedTargetCharacterCtrl()
{
	...
	if (*(uint64_t*)(g_gameManagerImp + 0x20) == NULL)
		return NULL;

	uint64_t camera_manager = *(uint64_t*)(g_gameManagerImp + 0x20);
...
{
	sendMessage = (oSendMessage)((uint64_t)g_moduleAddr + 0x5EBE70);
	taeLookup = (oTaeLookup)((uint64_t)g_moduleAddr + 0xB3F3C0);
}