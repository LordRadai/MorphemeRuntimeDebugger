...
EventTrackEditor::EventTrack::EventTrack(Morpheme::EventTrackDef* src, float len, bool discrete)
{
    ...
    {
        this->m_event[i].m_frameStart = Math::timeToFrame(src->m_trackData[i].m_startTime * len, 60);
        this->m_event[i].m_duration = Math::timeToFrame(src->m_trackData[i].m_duration * len, 60);
...
void EventTrackEditor::EventTrack::SaveEventTrackData(float len)
{
    ...
    {
        this->m_source->m_trackData[i].m_startTime = Math::frameToTime(this->m_event[i].m_frameStart, 60) / len;
        this->m_source->m_trackData[i].m_duration = Math::frameToTime(this->m_event[i].m_duration, 60) / len;
...
EventTrackEditor::EventTrackEditor() 
{
    ...
    INIReader reader(".//MorphemeNetworkInspector//res//color//eventrack.ini");
